...
function Card({ quiz, deletequiz }) {
  function handleQuiz(id) {
    deletequiz(id);
  }
  return (
    <>
      <div className="col-sm" id={quiz.id}>
        <div className="card " style={{ width: "18rem" }}>
          <button
            style={{ width: "38px", outline: "none", border: "none" }}
            onClick={(e) => handleQuiz(quiz.id)}
          >
            ❌
          </button>
          <Link to={`/JQuiz/${quiz.id}`}>
            ...
            <div className="card-body">
              <p className="card-text text-center">{quiz.title}</p>
            </div>
          </Link>
        </div>
      </div>
